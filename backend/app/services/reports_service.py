...
<body>
  <div class="container">
    ...
    {html_body}
    <div class="footer">
      Generated automatically by Autonomous Competitive Intelligence Agent Network • {datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M GMT")}
    </div>
  </div>
</body>
</html>"""
...
def render_pdf_report(report_id: str, competitor_name: str, markdown_content: str) -> str:
    ...
    try:
        ...
        pdf.cell(
            pdf.epw, 5,
            _clean_latin1(f"Generated by Autonomous Competitive Intelligence Agent Network  |  {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M GMT')}"),
            align="C",
        )
...
def send_custom_price_alert_webhook(
    ...
    text = (
        ...
        f"• *Detected At*: {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M GMT')}"
    )
