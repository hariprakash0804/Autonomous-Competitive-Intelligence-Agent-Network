...
def scrape_url(url: str, timeout_sec: float = 5.0) -> Dict[str, Any]:
    """
    Synchronous HTTP scraper with strict 3s connect / 5s read timeout guard.
    Fails fast on Cloudflare/bot blocks to keep total pipeline execution under 20s.
    """
    headers = {"User-Agent": DEFAULT_USER_AGENT}
    timeout_config = httpx.Timeout(timeout_sec, connect=3.0)
    
    try:
        with httpx.Client(headers=headers, follow_redirects=True, timeout=timeout_config) as client:
        ...
        return {
            ...
            "content_hash": "",
            "is_stale": True,
            "stale_reason": f"Network error or timeout: {str(exc)}",
            "status_code": 0,
        }
...
async def scrape_url_async(url: str, timeout_sec: float = 5.0) -> Dict[str, Any]:
