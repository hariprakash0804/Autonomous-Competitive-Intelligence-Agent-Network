...
import { DollarSign, TrendingUp, TrendingDown, ArrowRightLeft, Search, Filter, Download, FileSpreadsheet, Image as ImageIcon } from 'lucide-react';
...
export default function PriceTimeline({ priceHistory, competitorName }) {
  ...
  const handleExportCSV = () => {
    ...
    const headers = ['Tier Name', 'Our Company Price ($)', `${compLabel} Price ($)`, 'Price Difference ($)'];
    ...
    link.setAttribute('download', `${compLabel.toLowerCase().replace(/\s+/g, '_')}_price_timeline.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
    setShowExportMenu(false);
    toast.success('Price Timeline data exported as CSV!', 'CSV Downloaded');
  };
  ...
  const handleExportPNG = async () => {
    ...
    try {
      ...
      const link = document.createElement('a');
      link.download = `${compLabel.toLowerCase().replace(/\s+/g, '_')}_price_chart.png`;
      link.href = dataUrl;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      toast.success('Complete Price Chart & all tier details exported as PNG image!', 'PNG Downloaded');
    } catch (err) {
            ...
            {showExportMenu && (
              <div className="absolute right-0 top-full mt-1 w-44 bg-slate-900 border border-white/10 rounded-xl shadow-2xl p-1.5 z-30 animate-scale-in text-xs space-y-1">
                <button
                  ...
                  className="w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-slate-300 hover:text-white hover:bg-emerald-500/10 transition-colors text-left"
                >
